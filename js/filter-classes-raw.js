...
class PageFilterClassesRaw extends PageFilterClasses {
	...
	static async pInitFeatLoadeds ({feat}) {
	...
	static async pInitOptionalFeatureLoadeds ({optionalfeature}) {
		return this._pInitGenericLoadeds({
			ent: optionalfeature,
	...
	static async pInitRewardLoadeds ({reward}) {
		return this._pInitGenericLoadeds({
			ent: reward,
			prop: "reward",
	...
	static async pInitCharCreationOptionLoadeds ({charoption}) {
		return this._pInitGenericLoadeds({
			ent: charoption,
			prop: "charoption",
	...
	static async _pInitGenericLoadeds ({ent, prop, page, propAncestorName}) {
		...
		const entityRoot = await Renderer.hover.pCacheAndGet(`raw_${prop}`, ent.source, ent.hash, {isCopy: true});
		const loadedRoot = {
			type: prop,
			entity: entityRoot,
			page,
