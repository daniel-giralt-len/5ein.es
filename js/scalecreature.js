	...
	_adjustSpellcasting (mon, crIn, crOut) {
		...
		mon.spellcasting.forEach(sc => {
			...
			if (sc.headerEntries) {
				...
				const mClasses = /(artificer|bard|cleric|druida|paladí|ranger|sorcerer|bruixot|wizard) spell(?:s)?/i.exec(outStr);
				if (mClasses) spellsFromClass = mClasses[1];
				else {
					const mClasses2 = /(artificer|bard|cleric|druida|paladí|ranger|sorcerer|bruixot|wizard)(?:'s)? spell list/i.exec(outStr);
