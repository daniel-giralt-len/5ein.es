...
(typeof module !== "undefined" ? global : window).ScaleCreature = {
	...
	_wepFinesse: ["daga", "dard", "rapier", "simitarra", "espasa curta", "whip"],
	...
	_adjustSpellcasting (mon, crIn, crOut) {
		...
		mon.spellcasting.forEach(sc => {
			...
			if (sc.headerEntries) {
				...
				const mClasses = /(artificer|bard|cleric|druida|paladí|ranger|sortiller|bruixot|wizard) spell(?:s)?/i.exec(outStr);
				if (mClasses) spellsFromClass = mClasses[1];
				else {
					const mClasses2 = /(artificer|bard|cleric|druida|paladí|ranger|sortiller|bruixot|wizard)(?:'s)? spell list/i.exec(outStr);
...
(typeof module !== "undefined" ? global : window).ScaleSummonCreature = {
	...
	_scale_hp (mon, toSpellLevel, state) {
		...
		mon.hp.special = mon.hp.special
			...
			// "equal the aberration's Constitució modifier + el teu modificador de llançament de conjurs + ten times the spell's level"
