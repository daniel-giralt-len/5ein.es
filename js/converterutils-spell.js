...
class SavingThrowTagger {
	static tryRun (sp, options) {
		sp.savingThrow = [];
		JSON.stringify([sp.entries, sp.entriesHigherLevel]).replace(/(Força|Destresa|Constitució|Intel·ligència|Saviesa|Carisma) saving throw/ig, (...m) => sp.savingThrow.push(m[1].toLowerCase()));
...
class AbilityCheckTagger {
	static tryRun (sp, options) {
		sp.abilityCheck = [];
		JSON.stringify([sp.entries, sp.entriesHigherLevel]).replace(/a (Força|Destresa|Constitució|Intel·ligència|Saviesa|Carisma) check/ig, (...m) => sp.abilityCheck.push(m[1].toLowerCase()));
		...
		MiscTagsTagger._WALKER.walk(
			[sp.entries, sp.entriesHigherLevel],
			{
				string: (str) => {
					...
					if (/punts de vida temporals/ig.test(str)) tags.add("THP");
