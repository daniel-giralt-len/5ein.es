...
function Renderer () {
	...
	this._renderString_renderTag = function (textStack, meta, options, tag, text) {
		// region Plugins
		const plugins = this._getPlugins("string", "tag");
		...
		switch (tag) {
			...
			default: {
				const {name, source, displayText, others, page, hash, hashPreEncoded, pageHover, hashHover, hashPreEncodedHover, preloadId, linkText} = Renderer.utils.getTagMeta(text, tag);
...
Renderer.utils = {
			...
			<th class="rnd-name ${opts.extraThClasses ? opts.extraThClasses.join(" ") : ""}" colspan="6" ${dataPart}>
				<div class="name-inner">
					<div class="flex-v-center">
						...
						${opts.controlRhs || ""}
						${ExtensionUtil.ACTIVE && opts.page ? `<button title="Send to Foundry (SHIFT for Temporary Import)" class="btn btn-xs btn-default btn-stats-name mx-2 mb-2 self-flex-end" onclick="ExtensionUtil.pDoSendStats(event, this)"><span class="glyphicon glyphicon-send"></span></button>` : ""}
					</div>
	...
	getTagMeta (text, tag) {
		switch (tag) {
			...
			default: return Renderer.utils._getTagMeta_generic(text, tag);
		}
	},

	_getTagMeta_generic (text, tag) {
		...
		switch (tag) {
					...
					if (type && value) {
						switch (type) {
							...
							case VeCt.HASH_SCALED_SUMMON: {
								...
								out.preloadId = Renderer.monster.getCustomHashId({name, source, _isScaledSummon: true, _scaledSummonLevel: scaledSpellNum});
								out.subhashes = [
			...
			case "@class": {
				...
				if (others.length) {
					...
					out.subhashes = [{key: "state", value: hoverSubhashObj.state, preEncoded: true}];
...
Renderer.class = {
	...
	getHitPointsAtFirstLevel (clsHd) { return clsHd ? `${clsHd.number * clsHd.faces} + el teu modificador de Constitució` : null; },
	getHitPointsAtHigherLevels (className, clsHd, hdEntry) { return className && clsHd && hdEntry ? `${Renderer.getEntryDice(hdEntry, "Hit die")} (or ${((clsHd.number * clsHd.faces) / 2 + 1)}) + el teu modificador de Constitució per ${className} level after 1st` : null; },
...
Renderer.spell = {
	...
	initClasses (spell) {
		...
		if (spell.level <= 5 && spell.classes && spell.classes.fromClassList && spell.classes.fromClassList.some(c => (c.name === Renderer.spell.STR_SORCERER || c.name === Renderer.spell.STR_WARLOCK || c.name === Renderer.spell.STR_WIZARD) && c.source === SRC_PHB)) {
			...
			// Soul de Rellotgeria doesn't get a cantrip
...
Renderer.monster = {
		...
		function getExampleSpells (maxSpellLevel, color) {
			const LVL_TO_COLOR_TO_SPELLS = {
				2: {
					...
					A: ["see invisibility", "boca màgica", "ceguera/sordera", "sleep", "detectar pensaments"],
				...
				3: {
					...
					S: ["sleet storm", "protegir de l'energia", "catnap|XGE", "localitzar objecte", "identificar", "Barraca Diminuta d'en Leomund"],
				},
				4: {
					B: ["vitriolic sphere|XGE", "sickening radiance|XGE", "Evard's black tentacles", "plaga", "fam d'Hadar"],
					W: ["escut de foc", "tempesta de gel", "sleet storm"],
				...
				6: {
					...
					A: ["scrying", "Vincle Telepàtic d'en Rary", "Dansa Irresistible de l'Otto", "recordar llegendes", "immobilitzar criatura", "somnis"],
				},
				7: {
					...
					C: ["symbol", "simulacrum", "invertir la gravetat", "projectar imatge", "Mà d'en Bigby", "mental prison|XGE", "seeming"],
	...
	getRenderedSenses (senses, isPlainText) {
		...
		const reSenses = new RegExp(`(^| |\\()(${["tremorsense", "visió a cegues", "truesight", "visió de foscor", ...Object.keys(BrewUtil.homebrewMeta?.senses || []).map(it => it.escapeRegexp())].join("|")})(\\)| |$)`, "gi");
...
Renderer.item = {
	...
	_createSpecificVariants (baseItems, genericVariants, opts) {
		...
		baseItems.forEach((curBaseItem) => {
			...
			if (curBaseItem.packContents) return; // e.g. "Fletxes (20)"
	...
	enhanceItem (item) {
		...
		if (item.type === "LA" || item.type === "MA" || item.type === "HA") {
			if (item.stealth) {
				Renderer.item._initFullEntries(item);
				item._fullEntries.push({type: "wrapper", wrapped: "The wearer has disadvantage on Destresa ({@skill Sigil}) checks.", data: {[VeCt.ENTDATA_ITEM_MERGED_ENTRY_TAG]: "type"}});
			}
