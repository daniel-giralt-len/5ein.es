...
class BaseParser {
	...
	static _isContinuationLine (entryArray, curLine, opts) {
		...
		if (/^(Força|Destresa|Constitució|Intel·ligència|Saviesa|Carisma)\s+saving throw/.test(cleanLine) && !opts.noSavingThrow) return true;
		// An ability score name
		if (/^(Força|Destresa|Constitució|Intel·ligència|Saviesa|Carisma)\s/.test(cleanLine) && !opts.noAbilityName) return true;
...
TagCondition._CONDITION_INFLICTED_MATCHERS = [
	...
	`begins to [^.!?]+? i està {@condition ([^}]+)}`, // e.g. `begins to turn to stone and is restrained`
	`saving throw[^.!?]+?or [^.!?]+? and remain {@condition ([^}]+)}`, // e.g. `or fall asleep and remain unconscious`
	`saving throw[^.!?]+?or be [^.!?]+? and land {@condition (prone)}`, // MM :: Gegant dels Núvols :: Fling
	`saving throw[^.!?]+?or be (?:pushed|pulled) [^.!?]+? and (?:\\w+ )?{@condition ([^}]+)}`, // MM :: Dragon Turtle :: Tail
	`the engulfed (?:creature|enemy) [^.!?]+? {@condition ([^}]+)}`, // MM :: Cub Gelatinós :: Engulf
	...
	`or take[^.!?]+?damage and (?:becomes?|is|be) {@condition ([^}]+)}`, // MM :: Quàsit || Claw
	...
	`on a failure[^.!?]+?${TagCondition.__TGT} [^.!?]+?\\. [^.!?]+?is also {@condition ([^}]+)}`, // MM :: Elemental d'Aigua :: Whelm
	...
	`if the ${TagCondition.__TGT} is a creature[^.!?]+?saving throw[^.!?]*?\\. On a failed save[^.!?]+?{@condition ([^}]+)}`, // VGM :: Mindwitness :: Rajos d'Ull
	`{@condition (?:[^}]+)} així, an? (?:${TagCondition.__TGT}|creature|enemy) [^.!?]+{@condition ([^}]+)}`, // VGM :: Vargouille :: Stunning Shriek
