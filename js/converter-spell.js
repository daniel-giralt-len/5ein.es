...
class SpellParser extends BaseParser {
	...
	static _setCleanRange (stats, line, options) {
		const getUnit = (str) => str.toLowerCase().includes("mile") ? "milles" : "peus";
		...
		if (range.toLowerCase() === "a vista") return stats.range = {type: "point", distance: {type: "a vista"}};
	...
	static _setCleanDuration (stats, line, options) {
		...
		if (dur.toLowerCase() === "instantani (see text)") return stats.duration = [{type: "instant", condition: "llegir text"}];
