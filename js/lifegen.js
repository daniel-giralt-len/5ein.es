...
const PARENTS_TIEFLING = [
	{min: 1, max: 4, display: "Both parents were humans, their infernal heritage dormant until you came along.", _races: ["Humà", "Humà"]},
	{min: 5, max: 6, result: () => { const p = RNG(2); return `One parent ${fmtChoice(p === 1 ? "mother" : "father")} was a tiefling and the other ${fmtChoice(p === 1 ? "father" : "mother")} was a human.` }, display: "One parent was a tiefling and the other was a human.", _races: ["Humà", "Tíflin"]},
	{min: 7, result: () => { const p = RNG(2); return `One parent ${fmtChoice(p === 1 ? "mother" : "father")} was a tiefling and the other ${fmtChoice(p === 1 ? "father" : "mother")} was a devil.` }, display: "One parent was a tiefling and the other was a devil.", _races: ["Diable", "Tíflin"]},
	{min: 8, result: () => { const p = RNG(2); return `One parent ${fmtChoice(p === 1 ? "mother" : "father")} was a human and the other ${fmtChoice(p === 1 ? "father" : "mother")} was a devil.` }, display: "One parent was a human and the other was a devil.", _races: ["Humà", "Diable"]},
];
...
const LIFE_EVENTS_ADVENTURES = [
	...
	{min: 71, max: 80, result: "You learned a great deal during your adventure. The next time you make an ability check or a saving throw, tens avantatge a tirades de the roll."},
...
const LIFE_EVENTS_BOONS = [
	...
	{min: 3, result: "You found a {@item cavall de munta}."},
	...
	{min: 7, result: "You once performed a service for a local temple. The next time you visit the temple, you can receive healing up to els teus punts de vida màxims."},
...
const LIFE_EVENTS_WEIRD_STUFF = [
	...
	{min: 5, result: "You were taken captive by a race of evil humanoids such as drow, kuo-toa, or quaggoths. You lived as a slave in the Sotafoscor until you escaped."},
	...
	{min: 10, result: () => `You met a demigod, an archdevil, an archfey, a demon lord, or a titan, ${choose("demigod", "archdevil", "gran fata", "demon lord", "titan")} and you lived to tell the tale.`, display: "You met a demigod, an archdevil, an archfey, a demon lord, or a titan, and you lived to tell the tale."},
...
const SUPP_CLASS = [
	...
	{min: 59, max: 64, result: "Paladí"},
	{min: 65, max: 70, result: "Explorador"},
	{min: 71, max: 84, result: "Rogue"},
	{min: 85, max: 89, result: "Sortiller"},
...
function onJsonLoad (lifeData, nameData) {
	...
	nameData.name.filter(it => it.source === SRC_XGE)
		.forEach(nameMeta => {
			...
			if (nameMeta.name === "Elf" || nameMeta.name === "Humà") {
			...
			} else if (nameMeta.name === "Tíflin") {
				const cpy = MiscUtil.copy(nameMeta);
				cpy.tables = cpy.tables.filter(it => it.option !== "Virtue");
				nameTables["diable"] = MiscUtil.copy(nameMeta);
			}
		});
}
