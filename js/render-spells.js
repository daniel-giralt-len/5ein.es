class RenderSpells {
	static $getRenderedSpell (sp, subclassLookup) {
		...
		renderStack.push(`
			...
			<tr><td colspan="6"><span class="bold">Duració de llançament: </span>${Parser.spTimeListToFull(sp.time)}</td></tr>
			<tr><td colspan="6"><span class="bold">Abast: </span>${Parser.spRangeToFull(sp.range)}</td></tr>
			<tr><td colspan="6"><span class="bold">Components: </span>${Parser.spComponentsToFull(sp.components, sp.level)}</td></tr>
			<tr><td colspan="6"><span class="bold">Duració: </span>${Parser.spDurationToFull(sp.duration)}</td></tr>
		...
		if (fromClassListVariant.length) {
			...
			if (current.length) {
				stackFroms.push(`<div><span class="bold">Classes Opcionals/Variants: </span>${Parser.spMainClassesToFull(current)}</div>`);
			}
			if (legacy.length) {
				stackFroms.push(`<div class="text-muted"><span class="bold">Classes Opcionals/Variants (legacy): </span>${Parser.spMainClassesToFull(legacy)}</div>`);
			}
		}
		...
		if (fromBackgrounds.length) {
			...
			stackFroms.push(`<div><span class="bold">Rerefons: </span>${fromBackgrounds.map(r => `${SourceUtil.isNonstandardSource(r.source) ? `<span class="text-muted">` : ``}${renderer.render(`{@background ${r.name}|${r.source}}`)}${SourceUtil.isNonstandardSource(r.source) ? `</span>` : ``}`).join(", ")}</div>`);
		}
		...
		if (sp.eldritchInvocations) {
			...
			stackFroms.push(`<div><span class="bold">Invocacions Èldritxes: </span>${sp.eldritchInvocations.map(r => `${SourceUtil.isNonstandardSource(r.source) ? `<span class="text-muted">` : ``}${renderer.render(`{@optfeature ${r.name}|${r.source}}`)}${SourceUtil.isNonstandardSource(r.source) ? `</span>` : ``}`).join(", ")}</div>`);
		}
		...
		if (sp._scrollNote) {
			...
			renderer.recursiveRender(`{@italic Note: Both the {@class guerrer||${Renderer.spell.STR_FIGHTER} (${Renderer.spell.STR_ELD_KNIGHT})|cavaller èldritx} and the {@class rogue||${Renderer.spell.STR_ROGUE} (${Renderer.spell.STR_ARC_TCKER})|entabanador arcà} spell lists include all {@class ${Renderer.spell.STR_WIZARD}} spells. Spells of 5th level or higher may be cast with the aid of a spell scroll or similar.}`, renderStack, {depth: 2});
